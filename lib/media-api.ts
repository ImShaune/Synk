...
export async function searchMovies(prefs: UserPreferences): Promise<MediaItem[]> {
    ...
    return shuffle(data.results as any[]).slice(0, 10).map((m) => ({
        id: `movie-${m.id}`,
        externalId: m.id,
...
export async function searchSeries(prefs: UserPreferences): Promise<MediaItem[]> {
    const [from, to] = prefs.yearRange
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return shuffle(data.results as any[]).slice(0, 10).map((s) => ({
        id: `series-${s.id}`,
        externalId: s.id,
...
export async function searchGames(prefs: UserPreferences): Promise<MediaItem[]> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return shuffle(data.results as any[]).slice(0, 10).map((g) => ({
        id: `game-${g.id}`,
        externalId: g.id,
        ...
        platforms: (g.platforms ?? []).map(
            (x: { platform: { name: string } }) => x.platform.name
        ),
...
export async function getMovieDetail(
    ...
    const director = (
        credits.crew as { job: string; name: string }[]
    ).find((c) => c.job === 'Director')?.name

    const trailerKey = (
        videos.results as { type: string; site: string; key: string }[]
    ).find((v) => v.type === 'Trailer' && v.site === 'YouTube')?.key

    const arPlatforms: string[] =
        (
            providers.results?.AR
                ?.flatrate as { provider_name: string }[] | undefined
