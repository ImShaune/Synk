...
import { Star, Clock, Heart, RotateCcw, Play, MapPin, Share2, Tv, User, Building } from 'lucide-react'
...
export function MatchResult({ result, onSave, onRetry, onReset, isSaved = false }: MatchResultProps) {
  ...
  function handleShare() {
    if (navigator.share) {
      navigator.share({
        title: 'MatchFlix',
        text: 'Mi match es ' + media.title + ' con ' + compatibilityScore + '% de compatibilidad!',
    ...
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto px-4 pb-12">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {media.genres.length > 0 && (
            <div className="col-span-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {media.genres.map((g) => (
                  <span key={g} className="text-xs px-2.5 py-1 rounded-full bg-white/8 border border-white/10 text-white/60">
                    {g}
                  </span>
                ))}
              </div>
          ...
          {media.director && (
            <div>
              ...
              <p className="text-white/70 text-sm font-medium">{media.director}</p>
            </div>
          )}
          ...
          {media.developer && (
            <div>
              ...
              <p className="text-white/70 text-sm font-medium">{media.developer}</p>
            </div>
          )}
          ...
          {media.studio && (
            <div>
              <p className="text-white/30 text-xs mb-1 flex items-center gap-1">
                <Building className="w-3 h-3" />
                {media.category === 'series' ? 'Canal / Red' : 'Estudio'}
              </p>
              <p className="text-white/70 text-sm font-medium">{media.studio}</p>
            </div>
          )}
          ...
          {media.duration && (
            <div>
              ...
              <p className="text-white/70 text-sm font-medium">{media.duration}</p>
            </div>
          )}
          ...
          {media.episodes && (
            <div>
              ...
              <p className="text-white/70 text-sm font-medium">{media.episodes}</p>
            </div>
          )}
      ...
      {media.synopsis && (
        <motion.div
          ...
          className="w-full glass rounded-2xl p-6 mb-4"
        >
          <p className="text-white/40 text-xs uppercase tracking-widest mb-3">De que trata?</p>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {media.platforms.map((p) => (
              <span key={p} className="text-sm px-3 py-1.5 rounded-xl bg-white/8 border border-white/10 text-white/70">
                {p}
              </span>
            ))}
          </div>
